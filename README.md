...
- **Tonality**: major only, diatonic scale degrees, correctly spelled. Chromatic alterations are limited to applied dominants (below) — no modal mixture, Neapolitan, or augmented sixths yet.
- **Chords**: major/minor/diminished triads, dominant seventh, with inversions (triads: root/6/64; V7: root/65/43/42).
- **Roman numerals**: I, ii, iii, IV, V, V7, vi, vii°, plus applied ("secondary") dominants V/x and V7/x for x in {ii, iii, IV, V, vi} — e.g. `V/V`, `V7/vi` — with `HarmonicFunction` (tonic/predominant/dominant) kept as a distinct concept from the numeral itself.
...
1. **On the full major-mode Bach chorale corpus, mokuren harmonizes most but not all of what it can attempt.** A 144-chorale baseline (all major-mode Riemenschneider chorales with a rest-free soprano, via `tools/music21_chorale_extractor.py`) found **91.7% coverage** (132/144), 0 hard-rule violations — up from 50.7% in the v0.1.0 baseline, after implementing applied dominants and fixing the soprano-range ceiling (item 6 below and `tasks/baseline-v0.1.0.md`'s 5 "voice range" failures, all traced to a soprano note on A5, one step above the old default of G5). Full detail and the per-chorale diff against v0.1.0: `BENCHMARK.md` and `tasks/baseline-v0.2.0-secondary-dominants.md`. Of the 12 remaining failures, 6 are search-exhausted (a wider beam finds a path — the vocabulary roughly doubling means more candidates compete for the same fixed beam width, see item 4) and 6 are still undiagnosed (likely chromatic tones the applied-dominant vocabulary doesn't cover — modal mixture, non-chord tones outside the V/x, V7/x set).
2. **`Melody` can't represent a rest**, so 75/371 chorales (20.2% of the full corpus) were excluded from the baseline above before harmonization was even attempted — a gap on the same order as minor mode (item 7) but a data-model limitation, not a theory one.
