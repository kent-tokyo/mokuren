...
1. **On the full major-mode Bach chorale corpus, mokuren harmonizes most but not all of what it can attempt.** A 144-chorale baseline (all major-mode Riemenschneider chorales with a rest-free soprano, via `tools/music21_chorale_extractor.py`) found **94.4% coverage** (136/144), 0 hard-rule violations — up from 50.7% in the v0.1.0 baseline, after implementing applied dominants and fixing the soprano-range ceiling and an applied-dominant-prolongation bug (item 6 below; full detail in `BENCHMARK.md` and `tasks/baseline-v0.2.0-secondary-dominants.md`). Of the 8 remaining failures: 6 are search-exhausted (a wider beam finds a path — the vocabulary roughly doubling means more candidates compete for the same fixed beam width, see item 4) and 2 are a genuine unfixed gap — a soprano note forced into a formal chordal-seventh role that must resolve by step, when the real melody leaps a third instead, most likely Bach using the note as a decorative non-chord/passing tone, which mokuren has no model for at all (every soprano note must currently be a full chord tone).
